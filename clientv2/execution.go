...
func (c *Client) ExecuteTransactionBlock(
	...
	for i, signature := range signatures {
		sigBytes, err := pb.SignatureBytes(signature)
		if err != nil {
	...
	resp, err := c.exec.ExecuteTransaction(ctx, &pb.ExecuteTransactionRequest{
		...
		ReadMask:    &fieldmaskpb.FieldMask{Paths: pb.ResponseReadMaskPaths(opts)},
	})
	if err != nil {
		return nil, fmt.Errorf("ExecuteTransactionBlock: %w", err)
	}
	response := resp.GetTransaction().ToInternalType(opts)
	if response == nil {
...
func (c *Client) DryRunTransaction(ctx context.Context, txBytes lib.Base64Data) (*types.DryRunTransactionBlockResponse, error) {
	...
	response := resp.GetTransaction().ToInternalType(dryRunOptions)
	if response == nil {
...
func (c *Client) simulate(
	...
	paths := pb.PrefixPaths(simulateReadMaskPrefix, pb.ResponseReadMaskPaths(options))
